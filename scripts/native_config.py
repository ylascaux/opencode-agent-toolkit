"""Dependency-free renderer for the current stable OpenCode 2 runtime."""
...
def permission_rules(value: Mapping[str, Any]) -> list[dict[str, str]]:
    rules: list[dict[str, str]] = []
    for action, values in value.items():
        resources = {"*": values} if isinstance(values, str) else values
        for resource, effect in resources.items():
            rules.append({
                "action": {"bash": "shell", "task": "subagent"}.get(action, action),
                "resource": resource,
                "effect": effect,
            })
    return rules
...
def render_config(specs: Mapping[str, Any]) -> dict:
    ...
    for name, spec in sorted(specs.items()):
        ...
        if delegated:
            parts.append(
                "Delegate to the smallest useful set of these children:\n"
                + "\n".join(f"- {child}: {specs[child].description}" for child in delegated)
            )
            parts.append(
                "Give each child a concrete goal and context; consume its result before "
                "starting dependent work. Parallelize only independent tasks."
            )
