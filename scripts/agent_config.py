...
def _children_by_parent(specs: dict[str, AgentSpec]) -> dict[str, list[str]]:
...
def _validate_graph(specs: dict[str, AgentSpec]) -> None:
...
def load_agents() -> dict[str, AgentSpec]:
    ...
    for directory in discover_agent_dirs():
        ...
        specs[name] = AgentSpec(
            ...
            opencode=copy.deepcopy(config["opencode"]),
...
def children_by_parent(specs: dict[str, AgentSpec]) -> dict[str, list[str]]:
    children = _children_by_parent(specs)
...
def task_permissions(name: str, specs: dict[str, AgentSpec]) -> dict[str, str]:
    children = children_by_parent(specs)[name]
    return {"*": "deny", **{child: "allow" for child in children}}


def v1_permissions(name: str, specs: dict[str, AgentSpec]) -> dict:
    return {"task": task_permissions(name, specs), **copy.deepcopy(specs[name].permissions)}


def v2_permissions(name: str, specs: dict[str, AgentSpec]) -> list[dict]:
    rules: list[dict] = []
...
def write_generated_sources(specs: dict[str, AgentSpec]) -> None:
...
def model_tiers(specs: dict[str, AgentSpec] | None = None) -> dict[str, str]:
