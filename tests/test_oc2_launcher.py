...
class Oc2LauncherTests(unittest.TestCase):
    def _make_toolkit(self, tmp_path: Path):
        ...
        for name in ("generate-config", "apply-memory", "apply-reliability", "resolve-models"):
            ...
            path.chmod(0o755)
        # Old settings must not change the selected native version.
    ...
    def _output_map(stdout: str) -> dict[str, str]:
        return {key: value for line in stdout.splitlines() if "=" in line
    ...
    def test_oc2_symlink_forces_v2_even_when_env_defaults_to_v1(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run([str(launcher), "run", "hello"], env=env, capture_output=True, text=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            output = self._output_map(result.stdout)
            self.assertEqual(output["binary"], "opencode2")
            self.assertEqual(output["major"], "2")
            self.assertEqual(Path(output["config"]).resolve(), (toolkit / "opencode.v2.jsonc").resolve())
    ...
    def test_explicit_v2_override_wins_over_env_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env.pop("OPENCODE_BIN", None)
            result = subprocess.run(["bash", str(toolkit / "scripts" / "opencode-agents"), "run", "hello"],
