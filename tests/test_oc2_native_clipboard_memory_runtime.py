...
class NativeClipboardMemoryRuntimeTests(unittest.TestCase):
    """Compatibility tests for explicitly invoked legacy Docker tooling."""
    ...
    def test_docker_runtime_prefers_matching_native_v1_client(self):
        ...
        self.assertNotIn("OAT_OC_CLIENT_MODE=", env)
        self.assertNotIn("OAT_OC_NATIVE_BIN=", env)
    ...
    def test_docker_runtime_prefers_matching_native_v2_client(self):
        ...
        self.assertNotIn("OAT_OC2_CLIENT_MODE=", env)
    ...
    def test_legacy_server_password_is_generated_locally_when_missing(self):
        runtime = (ROOT / "scripts" / "docker-runtime").read_text()
        env = (ROOT / ".env.example").read_text()
